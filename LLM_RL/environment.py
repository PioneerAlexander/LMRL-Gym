...
class TextTrajectory:
    ...
    def __post_init__(self):
        ...
        assert all([r == 0.0 for r, t in zip(self.reward, self.text_history) if
                    not t.is_action]), "reward for non-actions texts should be 0.0"
...
class BatchedTextEnv(ABC):
    @abstractmethod
    def step(self, text_history: List[Optional[TextHistory]], done: Optional[List[bool]] = None) -> List[
        Optional[Tuple[TextHistory, float, bool]]]:
        pass

    @abstractmethod
    def reset(self, seed: Optional[List[Optional[int]]] = None, options: Optional[List[Optional[Dict]]] = None) -> List[
        TextHistory]:
        pass
...
class TextEnvToBatchedTextEnv(BatchedTextEnv):
    ...
    def step(self, text_history: List[Optional[TextHistory]], done: Optional[List[bool]] = None) -> List[
        ...
        assert len(text_history) == len(
            self.batch_env_copies), 'batch size must be the same as the number of environments initalized'
        if done is None:
...
class BatchedTextPolicy(ABC):
    @abstractmethod
    def act(self, text_history: List[Optional[TextHistory]], done: Optional[List[bool]] = None) -> List[
        Optional[TextHistory]]:
        pass
...
class TextPolicyToBatchedTextPolicy(BatchedTextPolicy):
    ...
    def act(self, text_history: List[Optional[TextHistory]], done: Optional[List[bool]] = None, as_q: bool = False, as_reweight: bool=False) -> List[
        Optional[TextHistory]]:
        # print(done)
        # print(text_history)
...
def interact_environment(
    ...
    while not all(done):
        ...
        text_history = policy.act(
            text_history + [(Text("", is_action=False),)] * npad,
            done=done + [True] * npad,
            as_q=as_q,
...
def text_env_eval(
        ...
        interaction_callback: Optional[
            Callable[[List[Tuple[TextHistory, TextHistory, TextHistory, float, bool]]], None]] = None,
        bsize: int = 1,
        verbose: bool = True,
        as_q: bool = False,
        as_reweight: bool = False,
    ...
    for _ in tqdm(range((n_rollouts + (bsize - 1)) // bsize), disable=not verbose):
        ...
        interaction_batch = interact_environment(
            ...
            env_seed=[None] * actual_bsize if seed_generator is None else [next(seed_generator) for _ in
                                                                           range(actual_bsize)],
...
class TokenHistory:
    ...
    def __post_init__(self):
        assert len(self.tokens.shape) == 1 and len(
            self.is_action.shape) == 1, '(tokens, is_action) must be 1 dimensional'
...
class TokenTrajectory:
    ...
    def __post_init__(self):
        ...
        assert not np.any(
            ((1 - self.is_action.astype(np.float32)) * self.reward) != 0.0), 'reward must be 0.0 if not an action'
