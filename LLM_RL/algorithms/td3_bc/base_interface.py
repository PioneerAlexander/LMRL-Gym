...
def td3_bc_loss(
    ...
    is_next_action = is_next_action.at[
        jnp.arange(0, is_next_action.shape[0], dtype=jnp.int32), jnp.argmax(is_next_action.astype(jnp.int32),
                                                                            axis=1)].set(False)
    ...
    # compute q loss
    q1_loss = (optax.l2_loss(q1sa_selected,
    ...
    transformed_q1sa_selected = jax.lax.stop_gradient(q1sa_selected / jax.lax.abs(q1sa_selected))
    bc_loss = ((-transformed_q1sa_selected + bc_weight * cross_entropy) * a_mask).sum() / n
