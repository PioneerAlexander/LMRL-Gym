    ...
    def load_train(
        ...
        def _step(
            ...
            def grad_loss(base_params: PyTree, q1_head_params: PyTree, q2_head_params: PyTree, v_head_params: PyTree,
                ...
                action = jnp.max(base_logits[:, :-1], axis=2)
                jax.debug.print("Actions shape: {x}", x=action.shape)
                jax.debug.print("Base logits shape: {x}", x=base_logits.shape)
                q1 = jnp.take_along_axis(q1_head_output[:, :-1], action, axis=2).squeeze(2)
                q2 = jnp.take_along_axis(q2_head_output[:, :-1], action, axis=2).squeeze(2)
                ...
                if next_token_ids is not None:
                    ...
                    last_next_token_idxs = (next_tokens_attention_mask.shape[1] - 1) - jnp.argmax(
                        jnp.flip(next_tokens_attention_mask, axis=1).astype(jnp.int32), axis=1)

                    final_next_token_h = next_token_target_base_model_output.hidden_states[-1][
                    ...
                    last_token_idxs = (attention_mask.shape[1] - 1) - jnp.argmax(
                        jnp.flip(attention_mask, axis=1).astype(jnp.int32), axis=1)
                    final_state_idxs = ((1 - dones) * last_action_idxs + dones * last_token_idxs).astype(jnp.int32)
                    target_q1_final = target_q1_full[
