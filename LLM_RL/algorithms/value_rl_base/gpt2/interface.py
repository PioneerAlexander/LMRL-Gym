...
from LLM_RL.algorithms.value_rl_base.gpt2.generation import GPT2ValueRLGeneration, GPT2ValueRLGenerationQ, \
    GPT2ValueRLGenerationReweight
...
class GPT2ValueRLInference(ValueRLInference):
    ...
    def load_inference(
        ...
        assert (pi_beta_model is None and pi_beta_params is None) or (
                    pi_beta_model is not None and pi_beta_params is not None)

        pi_beta_params_partition_spec = PS() if pi_beta_params is None else match_partition_rules(
            pi_beta_model.config.get_partition_rules(), pi_beta_params)
        ...
        q2_head_params_partition_spec = PS() if q2_head_params is None else match_partition_rules(
            q_head_model.config.get_partition_rules(), q2_head_params)
        v_head_params_partition_spec = PS() if v_head_params is None else match_partition_rules(
            v_head_model.config.get_partition_rules(), v_head_params)

        generator = None
        generator_q = None
        generator_reweight = None
        if pi_beta_params is None:
            ...
            generator_q = GPT2ValueRLGenerationQ(
                base_model_config=base_model.config,
                pi_beta=pi_beta_model,
                value_base=base_model,
                q_head=q_head_model,
            ...
            generator_reweight = GPT2ValueRLGenerationReweight(
                base_model_config=base_model.config,
                pi_beta=pi_beta_model,
                value_base=base_model,
                q_head=q_head_model,
        ...
        def _generate(
            ...
            if generator is not None:
                output = generator.generate(
                    ...
                    prng_key=prng_key,
                    generation_config=StreamingGenerationConfig.from_dict(
                ...
                raise ValueError('The inference type is not specified.')
        ...
        def _generate_reweight(
            ...
            output_reweight = generator_reweight.generate(
                input_ids=input_ids,
                attention_mask=attention_mask,
                params=(pi_beta_params, base_params, q1_head_params, q2_head_params),
                prng_key=prng_key,
        ...
        @partial(
            ...
            out_shardings=ValueRLForwardOutput(
                ...
                q1=NamedSharding(mesh, PS(("dp", "fsdp"), None, None)) if dp_shard_logits else NamedSharding(mesh,
                                                                                                             PS()),
                q2=NamedSharding(mesh, PS(("dp", "fsdp"), None, None)) if (
                        dp_shard_logits and q2_head_params is not None) else NamedSharding(mesh, PS()),
        ...
        def _forward(
            ...
            base_output = base_output.replace(
                logits=base_output.logits.at[:, :, base_model.config.unpadded_vocab_size:].set(-float('inf')))
            ...
            if dp_shard_logits:
                base_output = base_output.replace(
                    logits=with_named_sharding_constraint(base_output.logits, mesh, PS(("dp", "fsdp"), None, None)))
        ...
        return cls(
            ...
            tokenizer=tokenizer,
            _generate=_generate,
            _generate_q=_generate_q,
            _generate_reweight=_generate_reweight,
            _forward=_forward,
        )
...
class GPT2ValuePolicy(ValueRLPolicy):
    def __init__(
            ...
            blocking_strategy: BlockingStrategy = BlockingStrategy(padding=Padding.LEFT, truncation=Truncation.LEFT,
                                                                   max_length=None),
    ...
    def act(
            self,
            text_history: List[Optional[TextHistory]],
            done: Optional[List[bool]] = None,
            as_q: bool = False,
            as_reweight: bool = False,
    ) -> Union[List[Optional[TextHistory]], Tuple[List[Optional[TextHistory]]]]:
        if done is None:
        ...
        model_outputs = self.inference.generate_from_str(
            ...
            trace=self.trace,
            as_q=as_q,
            as_reweight=as_reweight,
        )

        def get_action(model_outputs):
        ...
        return get_action(model_outputs)
