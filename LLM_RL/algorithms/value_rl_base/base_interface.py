...
from LLM_RL.algorithms.value_rl_base.gpt2.generation import GPT2ValueRLGenerationOutput
...
class ValueRLInference(struct.PyTreeNode):
    ...
    _generate_q: Callable = struct.field(pytree_node=False)
    _generate_reweight: Callable = struct.field(pytree_node=False)
    ...
    def generate(
        ...
        if as_q:
            ...
            return self._generate_reweight(
                self.pi_beta_params,
                self.base_params,
                self.q1_head_params,
                self.q2_head_params,
    ...
    def generate_from_str(
            ...
            as_q: bool = False,
            as_reweight: bool = False,
    ) -> Union[GenerationFromStrOutput, Tuple[GenerationFromStrOutput, GenerationFromStrOutput]]:
        ...
        def get_output_from_logits(outputs):
        ...
        # process outputs
        return get_output_from_logits(outputs)
    ...
    def forward_from_str(
            self,
            input_strs: List[str],
            blocking_strategy: BlockingStrategy = BlockingStrategy(padding=Padding.RIGHT, truncation=Truncation.RIGHT,
                                                                   max_length=None),
            output_attentions: Optional[bool] = None,
