...
class CQLDataset(Dataset):
    ...
    @classmethod
    def from_cql_data_list(
        cls, 
        cql_data_list: List[CQLData], 
        tokenizer: PreTrainedTokenizerBase, 
        blocking_strategy: BlockingStrategy, 
    ) -> CQLDataset:
        
        data = CQLData.block(cql_data_list, blocking_strategy, tokenizer)

        return cls(**data)

class _CQLIteratorDataset:
    def __init__(self, cql_data: Iterator[Dict[str, np.ndarray]]):
        self.cql_data = cql_data

    def __next__(self):
        item = next(self.cql_data)
        return {
...
class CQLIterableDataset(IterableDataset):
    def __init__(self, cql_data: Iterable[Dict[str, np.ndarray]]):
        self.cql_data = cql_data
    
    def __iter__(self):
        return _CQLIteratorDataset(iter(self.iql_data))
    
    @classmethod
    def from_cql_data_iterable(
        cls, 
        cql_data: Iterable[CQLData], 
        tokenizer: PreTrainedTokenizerBase, 
        blocking_strategy: BlockingStrategy, 
    ) -> CQLIterableDataset:
        ...
        class _TokensIterable(Iterable):
            def _tokens_generator(self):
                for item in cql_data:
                    yield jax.tree_util.tree_map(lambda x: x[0], CQLData.block([item], blocking_strategy, tokenizer))
